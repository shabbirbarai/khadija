...
function showData() {
    for (var i = 0; i < items.length; i++) {
        document.getElementById("mainDiv").innerHTML += `<div id=` + i + ` class="image"  onmouseout="hideButtons(` + i + `)" onmouseover="showButtons(` + i + `)"><h4>` + items[i].name + `</h4><h4 class="price">Rs: ` + items[i].price + `/-</h4><img src="./images/` + items[i].image + `" alt="" class="imageSize" ><div class="btn1"><button class="button" onclick="decrementItem(` + i + `)"><i class="fa fa-minus fa-3x"></i></button><button class="button" onclick="incrementItem(` + i + `)"><i class="fa fa-plus fa-3x"></i></button></div></div>`;
    }
}
...
function countTotalItems() {
 ...
 function removeItem() {
    ...
    elementPosition > -1 ? whishListItems.splice(elementPosition, 1) : ture;
